...
def mt_main(mode,training_iters,model_name):
    ...
    if mode=="preprocessing":
        ...
        data = { split:preprocessing.prepareMTData(cur_data) for split,cur_data in list(data_seq.items())  }
        for split,split_data in list(data.items()):
            #print "Split: ",split
            inp,dinp,dout,dout_inp_matches = split_data
            #print inp.shape, dinp.shape, dout.shape, dout_inp_matches.shape
        #print "------------------------------------------------------------------------"
        #print ""
        pickle.dump(data,open(data_src + "data.obj","wb"))
    ...
    if params['pretrained_embeddings']:
        ...
        file = open(params['pretrained_embeddings_path'],"rb")
        pretrained_embeddings = pickle.load(file,encoding='latin1')
        ...
        not_found_count = 0
        for token,idx in list(word_to_idx.items()):
    ...
    if mode=='train' or mode=="debug":
        ...
        _ = rnn_model.getModel(params, mode='training',reuse=False, buckets=buckets)
    ...
    ## python mt_main.py <mode> <model path> <inference type> <sentence>
    elif mode == "chatTest":
        saved_model_path = sys.argv[2]
        print(("saved_model_path = ", saved_model_path))
        inference_type = sys.argv[3]  # greedy / beam
        ...
        rnn_model = solver.Solver(params, buckets=None, mode='inference')
        _ = rnn_model.getModel(params, mode='inference', reuse=False, buckets=None)
        print("Running Test sentence")

        ## Get the sentence
        sentence = sys.argv[4]
        print("Sentence : " , sentence)

        chatTest_encoder_inputs = preprocessing.loadChatTestSentence(sentence)

        chatTest_decoder_outputs = rnn_model.solveChatTest(params,chatTest_encoder_inputs,preprocessing.idx_to_word)

        print(chatTest_decoder_outputs)
        reverse_vocab = preprocessing.idx_to_word
        ret = ""
        for i, row in enumerate(chatTest_decoder_outputs):
            for val in row:
                if val == 2:
                    break
