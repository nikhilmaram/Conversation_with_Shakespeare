...
class PreProcessing:
	...
	def loadVocab(self, split):
		...
		inp_src = config.data_dir + split + ".modern" + ".nltktok" #".modern"
		out_src = config.data_dir + split + ".original" + ".nltktok" #".original"
	...
	def pruneVocab(self, max_vocab_size):
		...
		top_items = sorted( list(word_counters.items()), key=lambda x:-x[1] )[:max_vocab_size]
	...
	def loadData(self, split):
		...
		print("After preprocessing",inputs[0])
	...
	def loadChatTestSentence(self,sentence):
		inp_data = [sentence]
		inputs = self.preprocess(inp_data)
		...
		return np.array(sequences_input)
