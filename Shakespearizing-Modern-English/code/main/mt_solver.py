...
class Solver:
    ...
    def getModel(self, config, buckets, mode='training', reuse=False ):
        ...
        if mode=='training':
            #########################
            print("===================================================")
            for bucket_num, bucket_dct in list(self.buckets.items()):
            ...
            print("In decoding batch size.......",config["batch_size"])
        ...
        print("Getting the model is done")
            #if configuration.use_pointer:
        ...
        for bucket_num,bucket in enumerate(self.buckets):
            ...
            while step <= training_iters:
                ...
                for j in range(num_of_batches):
                    feed_dict_cur = {}
                    consider_iter = False
                    for k,v in list(feed_dct.items()):
                        if((j+1)*batch_size <n):
    ...
    def solveChatTest(self,config,encoder_inputs,reverse_vocab,sess=None,print_progress=False,inference_type="greedy"):
        if print_progress :
            print("In solveChatTest ..........................")
        if sess == None:
            print("Am I creating model again ???................")
            sess = tf.Session()
            saver = tf.train.Saver()
            saved_model_path = config['saved_model_path']
        ...
        for i in range(num_batches):
            if print_progress:
                print("i= ",i)
            encoder_inputs_cur = encoder_inputs

            if inference_type == "greedy":
                ## You need to use self.runInference Over here, but just implementing it functionality
                model_obj = self.model_obj
                feed_dct = {model_obj.token_lookup_sequences_placeholder_inference: encoder_inputs_cur}
                decoder_outputs_inference, encoder_outputs, alpha_inference = np.array(
                    sess.run([self.decoder_outputs_inference, self.encoder_outputs, self.alpha_inference],
                             feed_dict=feed_dct))  # timesteps, N

                decoder_outputs_inference = np.transpose(decoder_outputs_inference)
