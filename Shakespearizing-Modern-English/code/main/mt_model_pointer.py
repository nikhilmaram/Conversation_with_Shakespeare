...
class RNNModel:
    ...
    def __init__(self, buckets_dict, mode='training', params={}):
        ...
        if mode == 'training':
            ...
            for bucket_num, bucket in list(buckets_dict.items()):
                max_sentence_length = bucket['max_input_seq_length']
                self.token_lookup_sequences_placeholder_list.append(tf.placeholder("int32", [None, max_sentence_length],
                                                                                   name="token_lookup_sequences" + str(
                                                                                       bucket_num)))  # token_lookup_sequences
                max_sentence_length = bucket['max_output_seq_length']
                self.masker_list.append(
                    tf.placeholder("float32", [None, max_sentence_length], name="masker" + str(bucket_num)))
                ...
                self.token_lookup_sequences_decoder_placeholder_list.append(
                    tf.placeholder("int32", [None, max_sentence_length],
                                   name="token_lookup_sequences_decoder_placeholder" + str(
                                       bucket_num)))  # token_lookup_sequences

                self.token_output_sequences_decoder_inpmatch_placeholder_list.append(
                    tf.placeholder("float32", [None, bucket['max_output_seq_length'], bucket['max_input_seq_length']],
    ...
    def getEncoderModel(self, config, bucket_num=-1, mode='training', reuse=False):
        ...
        token_lookup_sequences_placeholder = None
        # placeholders
        if mode == 'training':
            token_lookup_sequences_placeholder = self.token_lookup_sequences_placeholder_list[bucket_num]
            print(self.token_lookup_sequences_placeholder_list[bucket_num])
            print(self.token_lookup_sequences_placeholder_list)
        elif mode == 'inference':
            self.token_lookup_sequences_placeholder_inference = token_lookup_sequences_placeholder = tf.placeholder(
                "int32", [None, max_sentence_length], name="token_lookup_sequences")  # token_lookup_sequences
        ...
        if reuse:
            ...
            self.encoder_token_emb_mat = token_emb_mat = self.initEmbeddings('emb_encoder', token_vocab_size,
                                                                             embeddings_dim, reuse=reuse,
                                                                             pretrained_embeddings=pretrained_embeddings,
                                                                             pretrained_embeddings_are_trainable=config[
                                                                                 'pretrained_embeddings_are_trainable'])
        inp = tf.nn.embedding_lookup(token_emb_mat, token_lookup_sequences_placeholder)
        print("EMbedding lookup shape : ", inp.shape)

        # run lstm
        outputs_tensor = self.encoderRNN(inp, lstm_cell_size, config['batch_size'], max_sentence_length, reuse,
                                         mode=mode)
        print("Encoding RNN is called")
    ...
    def attentionLayer(self, encoder_vals, h_prev, sentinel=None, reuse=False):  ## pointer network layer
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=reuse):
            ...
            encoder_vals_expanded = tf.concat([sentinel_expanded, encoder_vals],
                                              axis=1)  # N, encoder_sequence_length+1, lstm_cell_size
    ...
    def runDecoderStep(self, lstm_cell, cur_inputs, prev_cell_output, state, encoder_outputs, sentinel, reuse=False):
        alpha, sentinel_weight, context = self.attentionLayer(encoder_outputs, prev_cell_output, sentinel,
                                                              reuse)  # alpha is (None, encoder_sequence_length)
        # inputs =  cur_inputs
    ...
    def initDecoderOutputVariables(self, lstm_cell_size, token_vocab_size):
        with tf.variable_scope('decoder_output', reuse=None) as scope:
            w_out = tf.get_variable('w_out', shape=[lstm_cell_size, token_vocab_size],
            ...
            w_context_out = tf.get_variable('w_context_out', shape=[lstm_cell_size, token_vocab_size],
                                            initializer=tf.random_normal_initializer(-1.0, 1.0))
    ...
    def getDecoderOutput(self, output, lstm_cell_size, token_vocab_size, out_weights, alpha_sentinel,
    ...
    def initEmbeddings(self, emb_scope, token_vocab_size, embeddings_dim, pretrained_embeddings_are_trainable=True,
        ...
        with tf.variable_scope(emb_scope, reuse=reuse):
            if pretrained_embeddings.all() != None:
                print("Not all embeddings are null")
                ...
                token_emb_mat = tf.concat([tf.zeros([1, embeddings_dim]), tf.slice(token_emb_mat, [1, 0], [-1, -1])],
                                          axis=0)
            else:
                token_emb_mat = tf.get_variable("emb_mat", shape=[token_vocab_size, embeddings_dim], dtype='float')
                # 0-mask
        ...
        for time_step in range(num_steps):
            if time_step == 0:
                inp = tf.ones([batch_size, 1],
                              dtype=tf.int32)  # start symbol index  #TO DO: get start index from config
            ...
            (cell_output, state), alpha_cur, sentinel_weight, context = self.runDecoderStep(lstm_cell=lstm_cell,
                                                                                            cur_inputs=inputs_current_time_step,
                                                                                            encoder_outputs=encoder_outputs,
                                                                                            prev_cell_output=cell_output,
                                                                                            sentinel=sentinel,
                                                                                            reuse=(time_step != 0),
                                                                                            state=state)
            cur_outputs = self.getDecoderOutput(cell_output, lstm_cell_size, token_vocab_size, out_weights,
        ...
        with tf.variable_scope("RNN"):
            if mode == 'training':
                ...
                for time_step in range(num_steps):
                    ...
                    (cell_output, state), alpha, sentinel_weight, context = self.runDecoderStep(lstm_cell=lstm_cell,
                                                                                                cur_inputs=inputs_current_time_step,
                                                                                                encoder_outputs=encoder_outputs,
                                                                                                prev_cell_output=cell_output,
                                                                                                sentinel=sentinel,
                                                                                                reuse=(time_step != 0),
                                                                                                state=state)
                    cur_pred = self.getDecoderOutput(cell_output, lstm_cell_size, token_vocab_size, out_weights,
                                                     (alpha, sentinel_weight), encoder_input_sequence, batch_size,
                                                     token_vocab_size, context)
                    pred.append(cur_pred)

                    cur_decoder_output_inpmatch_sequence = decoder_output_inpmatch_sequence[:, time_step,
                                                           :]  # N,inp_seq_length
                    # alpha: N, inp_seq_length
                    cur_sentinel_attention_loss = tf.reduce_sum(alpha * cur_decoder_output_inpmatch_sequence,
                                                                axis=1)  # N
            ...
            # self.vals = vals
                ...
                outputs, alpha = self.greedyInferenceModel(
                    params)  # self.beamSearchInference(params)  #self.greedyInferenceModel(params)
    ...
    def getDecoderModel(self, config, encoder_outputs, is_training=False, mode='training', reuse=False, bucket_num=0):
        ...
        # placeholders
        if mode == 'training':
            token_lookup_sequences_decoder_placeholder = self.token_lookup_sequences_decoder_placeholder_list[
                bucket_num]
        ...
        if reuse:
            ...
            if share_embeddings:
                ...
                self.decoder_token_emb_mat = token_emb_mat = self.initEmbeddings(emb_scope, token_vocab_size,
                                                                                 embeddings_dim, reuse=reuse,
                                                                                 pretrained_embeddings=pretrained_embeddings,
                                                                                 pretrained_embeddings_are_trainable=
                                                                                 config[
        ...
        with tf.variable_scope('decoder', reuse=reuse):
            ...
            if mode == 'inference':
                params = {k: v for k, v in list(config.items())}
                ...
                pred_for_loss = tf.log(
                    pred)  # since sparse_softmax_cross_entropy_with_logits takes softmax on its own as well
                ...
                if is_training:
                    ...
                    cost = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred_masked,
                                                                          labels=token_output_sequences_placeholder)  # token_output_sequences_placeholder is N,timesteps. cost will be N, timesteps
